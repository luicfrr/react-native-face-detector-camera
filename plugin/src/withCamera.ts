...
const pkg = require( 'react-native-face-detector-camera/package.json' )
...
const gradleMaven = [
  `def faceDetectorCameraMavenPath = new File(["node", "--print", "require.resolve('react-native-face-detector-camera/package.json')"].execute(null, rootDir).text.trim(), "../android/maven")`,
  `allprojects { repositories { maven { url(faceDetectorCameraMavenPath) } } }`,
].join( '\n' )
...
export function addCameraImport( src: string ): MergeResults {
  return appendContents( {
    tag: 'face-detector-camera-import',
    src,
    newSrc: gradleMaven,
    comment: '//',
  } )
...
} | void> = ( config, { cameraPermission = true } = {} ) => {
